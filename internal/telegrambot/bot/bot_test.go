...
type trainerBot interface {
	ProcessCommand(ctx context.Context, b telegrambot.BotAPI, update *models.Update)
	ProcessMessage(ctx context.Context, b telegrambot.BotAPI, update *models.Update)
}
...
func TestTrainerBot_MultiplyScenario(t *testing.T) {
	...
	to.botAPI.EXPECT().SendMessage(mock.Anything, mock.Anything).Run(func(ctx context.Context, params *gotgbot.SendMessageParams) {
...
func TestTrainerBot_ProcessStartCommand_SendMessageCorrectly(t *testing.T) {
	...
	to.trainerBot.ProcessCommand(to.ctx, to.botAPI, &update)
...
func message(t *testing.T, to *testObjects, text string) {
	...
	to.trainerBot.ProcessMessage(to.ctx, to.botAPI, &update)
}
